if(typeof(ngutils)===typeof(undefined)){var ngutils={};(function($){ngutils.alert=function(msg_or_params){var params={message:'something happened',title:null,icon:'info',buttonText:'OK',callback:null,callbackThisArg:this};if(typeof msg_or_params==='string'){params.message=msg_or_params;}else if(typeof msg_or_params==='object'){for(let i in msg_or_params){params[i]=msg_or_params[i];}}
let template=$('#_ng_alert_template').html().trim();if(!template||template.length<1){window.alert(params.message);if(params.callback){params.callback.call(params.callbackThisArg);return;}}
let $template=$(template);if(!params.title){$template.find('[data-elem="header"]').remove();}
else{$template.find('[data-elem="title"]').text(params.title);if(params.icon){$template.find('[data-elem="title"]').addClass(params.icon);}}
$template.find('[data-elem="message"]').text(params.message);let $button=$template.find('[data-elem="button"]');if(params.buttonText){$button.text(params.buttonText);}
$button.click(function(){if(params.callback){params.callback.call(params.callbackThisArg);}
bo.unlock();bo.hide();});let bo=new ngutils.blackout({remove_on_hide:true});bo.lock();bo.show($template);};ngutils.confirm=function(msg_or_params){var params={message:'Are you sure?',title:null,icon:'question',confirmButtonText:'Confirm',confirmCallback:null,confirmCallbackThisArg:this,cancelButtonText:'Cancel',cancelCallback:null,cancelCallbackThisArg:this};if(typeof msg_or_params==='string'){params.message=msg_or_params;}else if(typeof msg_or_params==='object'){for(let i in msg_or_params){params[i]=msg_or_params[i];}}
let template=$('#_ng_confirm_template').html().trim();if(!template||template.length<1){if(window.confirm(params.message)){if(params.confirmCallback){params.confirmCallback.call(params.confirmCallbackThisArg);}}else{if(params.cancelCallback){params.cancelCallback.call(params.cancelCallbackThisArg);}}
return;}
let $template=$(template);if(!params.title){$template.find('[data-elem="header"]').remove();}
else{$template.find('[data-elem="title"]').text(params.title);if(params.icon){$template.find('[data-elem="title"]').addClass(params.icon);}}
$template.find('[data-elem="message"]').text(params.message);let $confirmButton=$template.find('[data-elem="button-confirm"]');if(params.confirmButtonText){$confirmButton.text(params.confirmButtonText);}
$confirmButton.click(function(){if(params.confirmCallback){params.confirmCallback.call(params.confirmCallbackThisArg);}
bo.unlock();bo.hide();});let $cancelButton=$template.find('[data-elem="button-cancel"]');if(params.cancelButtonText){$cancelButton.text(params.cancelButtonText);}
$cancelButton.click(function(){if(params.cancelCallback){params.cancelCallback.call(params.cancelCallbackThisArg);}
bo.unlock();bo.hide();});let bo=new ngutils.blackout({remove_on_hide:true});bo.lock();bo.show($template);};ngutils.alertAsync=function(msg_or_params){return new Promise(function(resolve,reject){ngutils.alert({message:msg_or_params.message,title:msg_or_params.title,icon:msg_or_params.icon,buttonText:msg_or_params.buttonText,callback:function(){resolve();}});});};ngutils.confirmAsync=function(msg_or_params){return new Promise(function(resolve,reject){ngutils.confirm({message:msg_or_params.message,title:msg_or_params.title,icon:msg_or_params.icon,confirmButtonText:msg_or_params.confirmButtonText,confirmCallback:function(){resolve();},cancelButtonText:msg_or_params.cancelButtonText,cancelCallback:function(){reject();}});});};var login_container,notification_bar,notification_shim;var www_root=PHP.get('www_root','/');var referrer=PHP.get('referrer',www_root);var is_mobile=PHP.get('ismobile')?true:false;var iOS=/iPad|iPhone|iPod/g.test(navigator.userAgent)&&!window.MSStream;ngutils.blacked_out=false;ngutils.screen_lock=false;ngutils.scrollBarWidth=0;ngutils.val_from_object=(obj,name,_default)=>{if(_default===undefined)_default=null;let parts=name.split('.');if(parts.length===1){return undefined===obj[name]?_default:obj[name];}
let ret=parts.reduce((prev,curr)=>prev?.[curr],obj);return undefined===ret?_default:ret;};ngutils.camelize=(str)=>{const a=str.toLowerCase().replace(/[-_\s.]+(.)?/g,(_,c)=>c?c.toUpperCase():'');return a.substring(0,1).toLowerCase()+a.substring(1);};ngutils.slugify=(str)=>{str=str.replace(/^\s+|\s+$/g,'');str=str.toLowerCase();var from="ÁÄÂÀÃÅČÇĆĎÉĚËÈÊẼĔȆÍÌÎÏŇÑÓÖÒÔÕØŘŔŠŤÚŮÜÙÛÝŸŽáäâàãåčçćďéěëèêẽĕȇíìîïňñóöòôõøðřŕšťúůüùûýÿžþÞĐđßÆa·/_,:;";var to="AAAAAACCCDEEEEEEEEIIIINNOOOOOORRSTUUUUUYYZaaaaaacccdeeeeeeeeiiiinnooooooorrstuuuuuyyzbBDdBAa------";for(var i=0,l=from.length;i<l;i++){str=str.replace(new RegExp(from.charAt(i),'g'),to.charAt(i));}
str=str.replace(/[^a-z0-9 -]/g,'').replace(/\s+/g,'-').replace(/-+/g,'-');return str;};ngutils.logOut=function(element){var action=$(element).attr('data-url');var $form=$('<form>').attr('action',action).attr('method','post');var $input=$('<input>').attr({type:'hidden',name:'userkey',value:PHP.get('uek')});$form.append($input);$('body').prepend($form);$form.submit();return false;};ngutils.checkDataAttributes=function(){$('*[data-exec-script]').click(function(){var handler=$(this).attr('data-exec-script');var params=$(this).attr('data-exec-params');try{if(params){params=JSON.parse(params);}}
...
var blob=new Blob(byteArrays,{type:contentType});return blob;};ngutils.textToClipboard=function(text){if(!navigator.clipboard){var textArea=document.createElement("textarea");textArea.value=text;textArea.style.top="0";textArea.style.left="0";textArea.style.position="fixed";document.body.appendChild(textArea);textArea.focus();textArea.select();try{var successful=document.execCommand('copy');var msg=successful?'successful':'unsuccessful';console.log('Fallback: Copying text command was '+msg);}catch(err){console.error('Fallback: Oops, unable to copy',err);}
...
ngutils.selectText=function($element){node=$element[0];if(document.body.createTextRange){const range=document.body.createTextRange();range.moveToElementText(node);range.select();}else if(window.getSelection){const selection=window.getSelection();const range=document.createRange();range.selectNodeContents(node);selection.removeAllRanges();selection.addRange(range);}else{console.warn("Could not select text in node: Unsupported browser.");}}
ngutils.ieHacks=function(){var ua=window.navigator.userAgent;var msie=ua.indexOf("MSIE ");if(msie>0||!!navigator.userAgent.match(/Trident.*rv\:11\./)){var outer=$('#outer-skin');var fixOuter=function(){$(outer).height($(document).height());};ngutils.event.addListener(ngutils.event.resize,fixOuter);fixOuter();}};$(function(){login_container=$('#user_login_container');notification_bar=$('#notification-bar');notification_shim=$('#notification-shim');ngutils.media.smartscale.autodetect();ngutils.element.autoCondense();ngutils.initScrollFixColumn();ngutils.ieHacks();ngutils.grids.init();ngutils.media.smartload.autodetect();ngutils.forms.fakeSelect.autodetect();ngutils.forms.selectLinks.autodetect();ngutils.forms.selectAltLabels.autodetect();var noscroll=function(e){if(ngutils.screen_lock){e.preventDefault();return false;}
return true;};$('#body-main').on('touchmove',noscroll);$('#notification-bar').on('touchmove',noscroll);});ngutils.ajaxOnClick=(event,link,complete_event,fail_event)=>{event.preventDefault();let $link=$(link);if($link.attr('data-confirm')&&!confirm($link.attr('data-confirm')))return;$.get($link.attr('href'),(result)=>{if(complete_event)ngutils.event.dispatch(complete_event,result);}).fail((result)=>{if(fail_event)ngutils.event.dispatch(fail_event,result);});};ngutils.onAjaxComplete=function(){setTimeout(function(){ngutils.media.smartscale.autodetect();ngutils.element.autoCondense();ngutils.media.smartload.autodetect();ngutils.forms.fakeSelect.autodetect();ngutils.forms.selectLinks.autodetect();ngutils.forms.selectAltLabels.autodetect();},3);};$(document).ajaxComplete(function(){ngutils.onAjaxComplete();});(()=>{ngutils.dom={};ngutils.dom.alter_attributes=(selector,callback)=>{let elements=ngutils.dom.nodes(selector),elements_length=elements.length;if(!elements_length){return;}
for(let i=0,len=elements.length;i<len;++i){callback(elements[i]);}};ngutils.dom.attr=(node,name)=>{if(node.hasAttribute(name)){return node.getAttribute(name);}
return null;};ngutils.dom.attr_either=(node,preferred_name,other_name)=>{let attr=ngutils.dom.attr(node,preferred_name);if(null!==attr)return attr;return ngutils.dom.attr(node,other_name);};ngutils.dom.nodes=(selector)=>{if(selector instanceof XPathResult){let node,nodes=[];while(node=selector.iterateNext())nodes.push(node);return nodes;}
return[...selector];};ngutils.dom.set_attributes=(collection,name,value)=>{if(collection instanceof jQuery){collection=ngutils.dom.nodes(collection.toArray());}
const callback=(node)=>{if(name instanceof String&&(value instanceof String||value instanceof Number)){node.setAttribute(name,value);return;}
if(name instanceof Function){return name(node);}
if(name instanceof Object&&undefined===value){for(const[k,v]of Object.entries(name)){node.setAttribute(k,v);}
return;}
throw"Don't know how to handle this.";};ngutils.dom.alter_attributes(collection,callback);};ngutils.dom.parse_text=(dom,options)=>{if(dom instanceof jQuery){dom=jQuery.get(0);}
options=options||{};ngutils.visual_links.add_from_dom(dom,options.visual_links);ngutils.dom.set_attributes(dom.querySelectorAll('a'),{'rel':'nofollow','target':'_blank'});ngutils.dom.set_attributes(dom.querySelectorAll('iframe'),{'allowfullscreen':'true'});ngutils.dom.set_attributes(dom.querySelectorAll('img'),(img)=>{img.setAttribute('data-user-image','1');});};})();(()=>{const startsWith=(haystack,needle)=>{return haystack.indexOf(needle)===0;};const endsWith=(haystack,needle)=>{let tail=haystack.substr(haystack.length-needle.length);return tail===needle;};ngutils.allowlist=function(config){if(!config.hasOwnProperty('domain')){throw"config must contain domain!";}
console.log(`got domain ${config.domain}`);this.domain=config.domain;let _this=this;let keys=['internal','allowed_params','banned_params','default_params','required_params','uris'];keys.forEach(key=>{_this[key]=typeof(config[key])!==typeof(undefined)?config[key]:null;});};ngutils.allowlist.prototype={getDomain:function(){return this.domain;},getAllowedParams:function(){return this.allowed_params;},getBannedParams:function(){return this.banned_params;},getDefaultParams:function(){return this.default_params;},getRequiredParams:function(){return this.required_params;},appendLocationTo:function(){},getUris:function(){return this.uris;},getCleanUrl:function(url){let urlObject=new URL(url);let givenParams=urlObject.searchParams;let newParams=new URLSearchParams(givenParams.toString());if(null!==this.getAllowedParams()){for(const[key,]of newParams.entries()){if(!this.getAllowedParams().includes(key)){newParams.delete(key);}}}
if(null===this.getAllowedParams()&&null!==this.getBannedParams()){for(const[key,]of newParams.entries()){if(this.getBannedParams().includes(key)){newParams.delete(key);}}}
if(null!==this.getDefaultParams()){for(const pair of this.getDefaultParams()){const[key,value]=pair.split('=');if(newParams.has(key))newParams.delete(key);newParams.append(key,value);}}
let to_return=`${urlObject.origin}${urlObject.pathname}`;if([...newParams.values()].length){to_return=`${to_return}?${newParams.toString()}`;}
return to_return;},hasRequiredParams:function(url){let urlObject=url instanceof URL?url:new URL(url);let required=this.getRequiredParams();if(null===required)return true;let givenParams=urlObject.searchParams;for(let i=0,len=required.length;i<len;++i){if(!givenParams.has(required[i])){return false;}}
return true;},hasValidUri:function(url){let urlObject=url instanceof URL?url:new URL(url);let uris=this.getUris();if(null===uris||uris.length===0){return true;}
const trimSlash=(str)=>{while(0===str.indexOf('/')&&str.length){str=str.substring(1);}
return str;};for(let i=0,len=uris.length;i<len;++i){let path=trimSlash(uris[i]);let given_path=trimSlash(urlObject.pathname);let qs=urlObject.searchParams.toString();if(qs.length){given_path=`${given_path}?${qs}`;}
if(startsWith(given_path,path)){return true;}}
return false;},validate:function(url){let urlObject=url instanceof URL?url:new URL(url);if(!this.hasValidUri(urlObject)){throw`You must include a valid URI (path) here for '${urlObject.hostname}'.`;}
if(!this.hasRequiredParams(urlObject)){let params='"'+this.getRequiredParams().join('", "')+'"';throw`Invalid query parameters passed - this URL needs certain parameters to work properly, and you haven't provided them. Missing one of: ${params}`;}
return this;},validateAndClean:function(url){let urlObject=url instanceof URL?url:new URL(url);return this.validate(urlObject).getCleanUrl(urlObject);}};ngutils.allowlist.prototype.constructor=ngutils.allowlist;ngutils.allowlist.applyTo=(nodeOrDom,allowLists,tagName,attributeName,removeIfNotFound)=>{tagName=tagName?tagName:'iframe';attributeName=attributeName?attributeName:'src';removeIfNotFound=typeof removeIfNotFound==='boolean'?removeIfNotFound:false;let nodes=[...nodeOrDom.getElementByTagName(tagName)],len=nodes.length;if(!len)return;const removeNodeIfNotFound=(i)=>{if(removeIfNotFound){console.log(`Must remove node ${i}, couldn't get URL from ${attributeName}`);nodes[i].remove();}};for(let i=0;i<len;++i){let node=nodes[i];let url=node[attributeName]?node[attributeName]:null;if(null===url){removeNodeIfNotFound(i);continue;}
let clean=ngutils.allowlist.restrict(url,allowLists);if(null===clean){removeNodeIfNotFound(i);continue;}
let clone=node.clone();clone[attributeName]=clean;node.parentNode.replaceChild(clone,node);}};ngutils.allowlist.fromUrlAndAllowlists=(url,allowLists,internal)=>{const urlObject=url instanceof URL?url:new URL(url);for(let i in allowLists){let domain=allowLists[i].domain;if(endsWith(urlObject.hostname,domain)&&(undefined===internal||(allowLists[i].internal?allowLists[i].internal:false)===internal)){return new ngutils.allowlist(allowLists[i]);}}
return null;};ngutils.allowlist.restrict=(url,allowLists)=>{let obj=ngutils.allowlist.fromUrlAndAllowlists(url,allowLists);if(null==obj)return null;return obj.getCleanUrl(url,obj);};ngutils.allowlist.to_modify=[];})();ngutils.input={lastX:null,lastY:null,getMouse:function(e){var input=e;if(!input){return{x:this.lastX,y:this.lastY};}
if(typeof(e.originalEvent)!=='undefined'&&typeof(e.originalEvent.touches)!=='undefined'){input=e.originalEvent.touches[0];}else if(typeof(e.changedPointers)!=='undefined'){input=e.changedPointers[0];}else if(typeof(e.pointers)!=='undefined'){input=e.pointers[0];}
...
ngutils.activeTasks={increment:0,tasks:{},$saving:null,activeTaskInterrupt:function(e){for(var i in ngutils.activeTasks.tasks){let message=ngutils.activeTasks.tasks[i].saving?'Your changes are still saving.\n\nAre you sure you want to leave?':'You have unsaved changes.\n\nAre you sure you want to leave the page?';e.returnValue=message;return message;}},checkTasks:function(){window.removeEventListener("beforeunload",ngutils.activeTasks.activeTaskInterrupt);let task=null;let saving=false;for(var i in ngutils.activeTasks.tasks){task=ngutils.activeTasks.tasks[i];if(task.saving)saving=true;}
...
ngutils.activeTasks.$saving.fadeIn();}else if(ngutils.activeTasks.$saving){ngutils.activeTasks.$saving.fadeOut();}},addTask:function(saving,$element){return new ngutils.activeTasks.task(saving,$element);},task:function(saving,$element){let _this=this;ngutils.activeTasks.increment++;this.name="task"+ngutils.activeTasks.increment;this.complete=false;this._saving=saving?true:false;ngutils.activeTasks.tasks[this.name]=this;if($element){this.blackout=ngutils.blackout.getByChild($element);}else{this.blackout=ngutils.blackout.getTop();}
this.completed=function(){delete ngutils.activeTasks.tasks[this.name];ngutils.activeTasks.checkTasks();_this.completed=function(){return false;};_this.complete=true;ngutils.event.dispatch(ngutils.event.taskCompleted,_this);return true;};ngutils.activeTasks.checkTasks();ngutils.event.dispatch(ngutils.event.taskStarted,this);},endTask:function(name){if(ngutils.activeTasks.tasks[name])ngutils.activeTasks.tasks[name].endTask();}};ngutils.activeTasks.task.prototype={_saving:false,get saving(){return this._saving;},set saving(s){this._saving=s?true:false;ngutils.activeTasks.checkTasks();}};ngutils.event={ajaxComplete:'ajax-completed',resize:'resize',closeall:'closeall',closeMenus:'closeMenus',newNotifications:'newNotifications',topMenuOpen:'topMenuOpen',userBlockUpdated:'user-block-updated',userBlocksUpdated:'user-blocks-updated',emoteSetChanged:'emote-set-changed',emoteSelectLoaded:'emote-select-loaded',tagsUpdated:'tag-collection-updated',taskStarted:'task-started',taskCompleted:'task-completed',blackoutClosed:'blackout-closed',listeners:[],addListener:function(event,listener,autoremove){if(typeof(ngutils.event.listeners[event])===typeof(undefined)){ngutils.event.listeners[event]=[];}
if(autoremove){const temp=listener;listener=()=>{temp();ngutils.event.removeListener(event,listener);};}
ngutils.event.listeners[event].unshift(listener);},removeListener:function(event,listener){if(typeof(ngutils.event.listeners[event])===typeof(undefined))return;var index=ngutils.event.listeners[event].indexOf(listener);if(index>=0)ngutils.event.listeners[event].splice(index,1);},removeAll:function(event){if(typeof(ngutils.event.listeners[event])===typeof(undefined))return;ngutils.event.listeners[event]=[];},dispatch:function(event,data){var i;if(typeof(ngutils.event.listeners[event])===typeof(undefined))return;var queue=ngutils.event.listeners[event].slice(0);for(i=0;i<queue.length;i++){if(queue[i](data)===false)return false;}
...
var _this=this;$(document).keyup(function(e){if(_this.visible&&e.key==='Escape'&&!_this.locked)_this.hide();});};ngutils.blackout.prototype={onLoad:function(callback,thisArg){this._onload=typeof(callback)==='function'?callback:null;this._onload_context=thisArg?thisArg:this;},onShow:function(callback,thisArg){this._onshow=typeof(callback)==='function'?callback:null;this._onshow_context=thisArg?thisArg:this;},onHide:function(callback,thisArg){this._onhide=typeof(callback)==='function'?callback:null;this._onhide_context=thisArg?thisArg:this;},onRemove:function(callback,thisArg){this._onremove=typeof(callback)==='function'?callback:null;this._onremove_context=thisArg?thisArg:this;},_load:function(method,url,data,jQuery_success){var _this=this;this.loaded=false;this.show(ngutils.blackout.spinner);if(typeof(data)==='function'){jQuery_success=data;data={};}
$[method](url,data,function(data,textStatus,jqXHR){_this.loaded=true;_this.show(data);ngutils.onAjaxComplete();if(typeof(jQuery_success)==='function'){jQuery_success.call(this,data,textStatus,jqXHR);}
if(_this._onload){_this._onload.call(_this._onload_context);}}).fail(function(jqXHR,textStatus,errorThrown){if(_this._onfail){_this._onfail.call(_this._onfail_context,jqXHR,textStatus,errorThrown);}});},load:function(url,data,jQuery_success){return this._load('getXD',url,data,jQuery_success);},post:function(url,data,jQuery_success){return this._load('postXD',url,data,jQuery_success);},createElements:function(){var $hover,$inner,$bookend,$bookshelf,$centered,$dimmer;var $body=$('body').first();var _this=this;$hover=$("<div>");$hover.addClass('blackout-hover');$hover.attr('data-id',this.id);$dimmer=$('<div>').addClass('page-dimmer');$hover.append($dimmer);var checkReset=function(id){if(id===_this.id){_this.initButtons();}};ngutils.event.addListener('blackout-refresh',checkReset);$hover.on('remove',function(){$hover.off('remove');ngutils.event.removeListener('blackout-refresh',checkReset);});$inner=$("<div>");$inner.addClass('blackout-inner');$bookend=$("<div>");$bookend.addClass('blackout-bookend');$bookshelf=$("<div>");$bookshelf.addClass('blackout-bookshelf');$centered=$("<div>");$centered.addClass('blackout-center');$bookshelf.append($bookend.clone().addClass('shimmed'));$bookshelf.append($centered);$bookshelf.append($bookend.clone().addClass('shimmed'));$inner.append($bookend.clone());$inner.append($bookshelf);$inner.append($bookend.clone());$hover.append($inner);$body.append($hover);_this.$elements.hover=$hover;_this.$elements.inner=$inner;_this.$elements.centered=$centered;var clickLock=false;$hover.click(function(){if(clickLock){clickLock=false;return;}
if(!_this.locked&&_this.remove_on_click_outside){_this.hide();}});$centered.click(function(e){e.stopPropagation();clickLock=false;});$centered.mousedown(function(e){clickLock=true;});},removeLayer:function(){var layered_at=ngutils.blackout.layers.indexOf(this);if(layered_at>-1)ngutils.blackout.layers.splice(layered_at,1);},getZIndex:function(){return(ngutils.blackout.layers.length*2)+ngutils.blackout.zIndex;},initButtons:function(){var _this=this;var $btn=$('[data-action="blackout-close"]',_this.$elements.inner);$btn.off('click');$btn.show();var in_blackout=$btn.parents('.blackout-hover').length>0;if(!in_blackout){$btn.remove();return;}
$btn.click(function(e){if(_this.locked)return;e.preventDefault();_this.hide();ngutils.event.dispatch('blackout-close',{'instance':_this,});});$btn.removeAttr('data-action');},hiding:false,timeout:null,showTemplate:function(selector){let template=$(selector).html();this.show(template);},show:function(data){var _this=this;this.hiding=false;this.removeLayer();ngutils.blackout.lockLastLayer();ngutils.blackout.layers.push(this);var $body=$('body').first();if(!_this.$elements.centered){_this.createElements();}
...
_this.initButtons();},unlinkTask:function(){for(var i in ngutils.activeTasks.tasks){if(ngutils.activeTasks.tasks[i].blackout==this){ngutils.activeTasks.tasks[i].blackout=null;}}},locked:false,lock_timeout:null,lock:function(time){if(this.lock_timeout)clearTimeout(this.lock_timeout);this.locked=true;if(time===true)time=500;if(typeof(time)==="number"){let _this=this;this.lock_timeout=setTimeout(()=>{_this.unlock();},time);}
return this;},unlock:function(){if(this.lock_timeout)clearTimeout(this.lock_timeout);this.locked=false;return this;},ignore_active_tasks:false,hide:function(callback){if(this.locked)return false;var _this=this;ngutils.event.dispatch('blackout-hide',{'instance':_this,});console.log('hide',_this);let activetask=null;let savingtask=false;if(!this.ignore_active_tasks){for(var i in ngutils.activeTasks.tasks){if(ngutils.activeTasks.tasks[i].blackout==this){if(ngutils.activeTasks.tasks[i].saving)savingtask=true;activetask=true;}}}
...
this.visible=false;ngutils.event.dispatch(ngutils.event.blackoutClosed,this);if(this.remove_on_hide)return this.remove();if(this._onhide)this._onhide.call(this._onhide_context);if(undefined!==callback){callback(this._onhide_context);}},toggle:function(){if(this.locked)return false;if(this.visible)this.hide();else this.show();},remove:function(){if(this.locked)return false;for(var e in this.$elements){this.$elements[e].remove();}
this.$elements={};this.loaded=false;this.visible=false;if(this._onremove)this._onremove.call(this._onremove_context);},getLayerElement:function(){var $hover;if(this.$elements&&this.$elements.hover)$hover=this.$elements.hover;return $hover;}};ngutils.blackout.prototype.constructor=ngutils.blackout;ngutils.blackout.zIndex=11000;ngutils.blackout.layers=[];ngutils.blackout.$background=null;ngutils.blackout.spinner='<em class="fa fa-spinner fa-spin" style="font-size: 2em"></em>';ngutils.blackout.templateFromOnClick=function(event,selector,options){event.preventDefault();if(!options){options={remove_on_hide:true};}
var blackout=new ngutils.blackout(options);blackout.showTemplate(selector);return false;}
ngutils.blackout.fromOnClick=function(event,$element,options,callback){event.preventDefault();$element=$($element);if(!$element.attr('href')){console.error('onClick element requires href attribute');return;}
...
x_left+=(star_width-xw)/ 2;$('body:first').append($xp_info);$xp_info.css({'top':x_top+'px','left':x_left+'px'});$({brightness:5}).animate({brightness:1},{duration:500,easing:'swing',step:function(){$xp_info.css({"-webkit-filter":'brightness('+this.brightness+')',"filter":'brightness('+this.brightness+')'});},complete:function(){$xp_info.fadeOut(100,function(){$xp_info.remove();});}});$xp_info.animate({top:(x_top-40)},{duration:500,easing:'swing'});};this.clickHandler=function(event,$clicked,record){if(invalid){event.preventDefault();$.postXD(PHP.get('www_root')+"/validate-content-permission",function(){});return;}
var score=$('#'+$clicked.attr('for')).val();if(this.castVote(event,score)){this.showFromLabel($clicked,record);}};this.castVote=function(event,score){score=parseInt(score,10);if(!$form.exists())return false;if(locked||score===last_score){this.getSelectedInput().prop('checked',false);$selected_input.prop('checked',true);return false;}
...
votebar.showFromLabel(over);};$element.on('touchstart',function(e){initTouch();checkTouch(e);});$element.on('touchmove',function(e){checkTouch(e);});$(document).off('touchend').on('touchend',function(e){if(!over)return;$('#'+over.attr('for'),$element).prop('checked',true);votebar.clickHandler(e,over,true);over=null;compare=[];$('label',$element).removeClass('hover');});}
...
return false;}
this.hideReactionOptions=function(){hideReactionOptions();};$reaction_button.click(function(e){return reaction_options_visible?hideReactionOptions(e):showReactionOptions(e);});if(!is_mobile)$element.hover(showReactionOptions,hideReactionOptions);function initEmoteButtons($container){var $add_button=$("a[data-action='add']",$container);var $remove_button=$("a[data-action='remove']",$container);$add_button.off('click');$remove_button.off('click');$add_button.on('click',function(e){e.stopPropagation();if(waiting)return false;var extra_id=parseInt($(this).attr('data-extra-id'));waiting=true;var ajax_url="/favorites/"+ngutils.fave_follow.types[fave_type]+"/add/"+button_key;if(typeof(extra_id)!==typeof(undefined)&&!isNaN(extra_id))ajax_url+="/extra/"+extra_id;var data={fave_type:ngutils.fave_follow.types[fave_type],emote_text:$(this).attr('title'),emote_class:$('span',$(this)).first().attr('class')};ngutils.fave_follow.refreshListeners(button_key,true,extra_id,data,_this);var params={userkey:user_key};if(PHP.get('ng_design'))params.___ng_design=PHP.get('ng_design');$.post(ajax_url,params,function(data){waiting=false;ngutils.fave_follow.refreshListeners(button_key,true,extra_id,data,_this);ngutils.reactions.refreshListeners(data.count_key,data,_this);},'json').fail(function(){waiting=false;ngutils.fave_follow.refreshListeners(button_key,false,extra_id,data,_this);});hideReactionOptions();return false;});$remove_button.on('click',function(e){e.stopPropagation();if(waiting)return false;waiting=true;var ajax_url="/favorites/"+ngutils.fave_follow.types[fave_type]+"/remove/"+button_key;var data={fave_type:ngutils.fave_follow.types[fave_type],emote_text:"React",emote_class:$('span',$(this)).first().attr('class')};ngutils.fave_follow.refreshListeners(button_key,false,0,data,_this);var params={userkey:user_key};if(PHP.get('ng_design'))params.___ng_design=PHP.get('ng_design');$.post(ajax_url,params,function(data){waiting=false;ngutils.fave_follow.refreshListeners(button_key,false,0,data,_this);ngutils.reactions.refreshListeners(data.count_key,data,_this);}).fail(function(){waiting=false;});hideReactionOptions();return false;});}
...
$input.on("click",doCopy);$a.on("click",doCopy);$input.on("keypress",function(){return false;});});};ngutils.sharebox.register=function($el){if(null===ngutils.sharebox.blackout){ngutils.sharebox.blackout=new ngutils.blackout({"remove_on_hide":true});}
$el.click(function(){var h=this.href;ngutils.sharebox.blackout.load(h,null,function(response){ngutils.sharebox.watch();});return false;});};ngutils.artSlideshow=class{static id=0;static slideshows={};static active=null;static create(name,$container){if(ngutils.artSlideshow.slideshows[name]===undefined){ngutils.artSlideshow.slideshows[name]=new ngutils.artSlideshow(name,$container);return ngutils.artSlideshow.slideshows[name];}
name=name+'-'+(ngutils.artSlideshow.id+1);return ngutils.artSlideshow.create(name,$container);}
static get(name){return ngutils.artSlideshow.slideshows[name];}
static getActive(){return ngutils.artSlideshow.slideshows[ngutils.artSlideshow.active];}
static clearActive(){ngutils.artSlideshow.active='global';return ngutils.artSlideshow.slideshows[ngutils.artSlideshow.active];}
constructor(name,$container){let _this=this;this.id=ngutils.artSlideshow.id++;this.name=name;this.slides=[];this.activeSlide=0;this.blackout=null;this._on_close=[];ngutils.artSlideshow.active=name;if($container){$container.find('[data-art-slide]').each(function(){new ngutils.artSlide($(this),_this);});}}
hasSlides(){return this.slides.length>0;}
addContainer($container){let _this=this;$container.find('[data-art-slide]').each(function(){new ngutils.artSlide($(this),_this);});return this;}
addElement($element){this.slides.push(new ngutils.artSlide($element,this));return this;}
getSlide(index){if(typeof index==='undefined'){index=this.activeSlide;}else{this.activeSlide=index;}
if(this.slides[index])this.slides[index].view();return this.slides[index];}
nextSlide(){this.activeSlide++;if(this.activeSlide>=this.slides.length){this.activeSlide=0;}
return this.getSlide();}
prevSlide(){this.activeSlide--;if(this.activeSlide<0){this.activeSlide=this.slides.length-1;}
return this.getSlide();}
load(endpoint){let _this=this;if(!this.blackout){this.blackout=new ngutils.blackout({remove_on_hide:true});this.blackout.onHide=function(){alert('hide');_this.blackout=null;}}
this.blackout.load(endpoint);return this;}
onClose(callback){if(!this._on_close.includes(callback)){this._on_close.push(callback);}
return this;}
offClose(callback){if(callback){if(this._on_close.includes(callback)){this._on_close.splice(this._on_close.indexOf(callback),1);}}else{this._on_close=[];}
return this;}};ngutils.artSlideshow.create('global');ngutils.artSlide=class{slug=null;$element=null;constructor($element,slideShow){if(!slideShow){slideShow=ngutils.artSlideshow.slideshows[ngutils.artSlideshow.active]?ngutils.artSlideshow.slideshows[ngutils.artSlideshow.active]:null;}
if(!slideShow){console.error("Missing or invalid slideshow object: ",slideShow);throw new Error("Couldn't find a slideshow to add this slide to.");}
this.slideShow=slideShow;if(this.slideShow){this.slideShow.slides.push(this);}
let _this=this;this.$element=$element;this.slug=null;if(!$element.attr('data-art-slide')){let $first=$element.find('[data-art-slide]').first();if($first.length){$element=$first;}}
if($element.attr('data-art-slide')){this.slug=$element.data('art-slide');}else if($element.attr('href')){this.slug=$element.attr('href').split("art/view/").pop();}else{$element('a').each(function(){if(!_this.slug&&$(this).attr('href').indexOf('art/view/')>-1){_this.slug=$(this).attr('href').split("art/view/").pop();}});}
if($element.data('endpoint')){this.endpoint=$element.data('endpoint');}else{this.endpoint=this.url;}
this.$handle=null;let handle=$("[data-item='icon']",$element);if(handle.length===0)handle=this.$element;this.setHandle(handle);}
setHandle($element){let _this=this;this.clearHandle();this.$handle=$element;this.$handle.on('click',function(e){e.preventDefault();e.stopPropagation();_this.view(e);});return this;}
clearHandle(){if(this.$handle){this.$handle.off('click');}
return this;}
view(){this.slideShow.activeSlide=this.slideShow.slides.indexOf(this);this.slideShow.load(this.endpoint);return this;}
get url(){return PHP.get('www_root')+'/preview/art/'+this.slug;}};ngutils.playlist_form={};ngutils.playlist_form.blackout=null;ngutils.playlist_form.watch=function(){var pod=$('#playlist_add');var myform=$('form',pod);myform.submit(function(){$(this).ajaxSubmit(function(response){if(response&&response.url){window.location.href=response.url;}else{console.log(response);ngutils.playlist_form.blackout.show(response);}});return false;});$('a.icon-close',pod).click(function(){ngutils.playlist_form.blackout.hide();});};ngutils.playlist_form.register=function($el){if(null===ngutils.playlist_form.blackout){ngutils.playlist_form.blackout=new ngutils.blackout({"remove_on_hide":true});ngutils.playlist_form.blackout._onfail=function(){ngutils.playlist_form.blackout.hide();};}
...
var fixed=$container[0]!==window;const onScroll=(e)=>{if(ngutils.element.isOnScreen($element,scroll_buffer,fixed)){$container.off('scroll',onScroll);if(callback)callback();}};const checkDOMChange=(data)=>{if(data.parentNode.contains($element.get(0))&&ngutils.element.isOnScreen($element,scroll_buffer,fixed)){ngutils.event.removeListener('ngutils-dom-change',checkDOMChange);if(callback)callback();}};ngutils.event.addListener('ngutils-dom-change',checkDOMChange);setTimeout(function(){if($element.length===1){if(_this.onReady()===false)return;$container.on('scroll',onScroll);onScroll();}else{console.warn("Invalid element");}},5);return this;};ngutils.element.autoCondense=function(){$('.condensed').each(function(){ngutils.element.condense($(this));});};ngutils.element.condense=function(element,more_text){var $condensed=$(element);if(!$condensed||$condensed.length<1)return;var $parent=$condensed.parent();var position=$parent.css('position').toLowerCase();if(position!='fixed'&&position!='absolute'&&position!='relative'){$parent.css('position','relative');}
...
return this;};this.toggle=function(){if(editable._editing)this.setEditing(false);else this.setEditing(true);return this;};this.setToggleButton=function(button,edit_text,noedit_text,callback,thisArg){$toggle_btn=$(button);if(!noedit_text)noedit_text=$toggle_btn.html();if(!edit_text)edit_text=noedit_text;toggle_params[0]=noedit_text;toggle_params[1]=edit_text;toggle_params[2]=typeof(callback)=='function'?callback:null;toggle_params[3]=thisArg?thisArg:$toggle_btn;this.setEditing(this._editing);$toggle_btn.click(function(e){editable.toggle();if(toggle_params[2]){return toggle_params[2].call(toggle_params[3],e);}else{return false;}});return this;};if(editing)this.setEditing(editable._editing);else if($e.hasClass('edit-on'))this.setEditing(true);else this.setEditing(false);};ngutils.element.editable.prototype={get editing(){return this._editing;},set editing(editing){this.setEditing(editing);}};ngutils.media={};ngutils.media.smartscale=function($element){var _this=this;if(!$element.attr('data-smart-scale'))return;var dimensions=$element.attr('data-smart-scale').split(",");var width=dimensions[0].indexOf("%")<0?parseInt(dimensions[0]):dimensions[0];var height=dimensions[1].indexOf("%")<0?parseInt(dimensions[1]):dimensions[1];var cw=null;$element.width(0);let $closest=$element.parent();var parent_selectors=['.scale-to-this','.blog-embed-left','p','.blog-post-body','.ql-body','.pod-body','.pod','.column','body'];var deep_check_at=parent_selectors.indexOf('.pod-body');var deep_check=false;var s,$p;for(s=0;s<parent_selectors.length;s++){$p=$element.closest(parent_selectors[s]);if($p&&$p.length>0&&($p.width()>0||$p.attr('data-scale-width'))){$closest=$p;break;}
if(deep_check_at>=s){deep_check=true;}}
if(!$closest.width()&&!$closest.attr('data-scale-width')){$element.css('width','100%');return false;}
$element.removeAttr('data-smart-scale');this.onResized=function(w,h){};function setDimensions(e){if($element.closest('html').length<1){$(window).off('resize',setDimensions);return;}
var pw;if($closest.attr('data-scale-width')){pw=parseFloat($closest.attr('data-scale-width'));}else{pw=$closest.width();if(deep_check){var $parent=$closest;var $child=$element;var padding=0;while($parent[0]!=$child[0]){let adjust=[];adjust.push(parseInt($child.css('paddingLeft')));adjust.push(parseInt($child.css('paddingRight')));adjust.push(parseInt($child.css('marginLeft')));adjust.push(parseInt($child.css('marginRight')));adjust.push(parseInt($child.css('borderLeftWidth')));adjust.push(parseInt($child.css('borderRightWidth')));adjust.forEach((a)=>{if(!isNaN(a))padding+=a;});$child=$child.parent();}
pw-=padding;}}
$closest.removeAttr('data-scale-width');if(pw===cw)return;var nw=width;var nh=height;if(typeof(nw)=='number'&&pw<width){nw=pw;if(typeof(nh)=='number'){var scale=pw / width;nh=Math.round(height*scale);}}
...
var $sync_element=$element.closest('.ng-img-container-sync');$media_element.addClass(css_class);$element.css(css_inline);$sync_element.css(css_inline);cw=nw;$element[0].dispatchEvent(new Event('rescaled'));_this.onResized(nw,nh);}
if(is_mobile){$(window).resize(setDimensions);}
setDimensions();this.refresh=function(){setDimensions();};this.setNewDimensions=function(w,h){if(w===width&&h===height)return;width=w;height=h;cw=null;setDimensions();};return this;};ngutils.media.smartscale.autodetect=function(){$('[data-smart-scale]').each(function(){new ngutils.media.smartscale($(this));});};ngutils.media.smartload=function($element){var self=this;var src=$element.attr('data-smartload-src');var y_buffer=400;var $container=$element.closest("div[class^='condensed']");if(!$container||$container.length<1)$container=null;if($container){var cropped_at=$container.offset().top+$container.innerHeight();if($element.offset().top<cropped_at)$container=null;}
...
ngutils.taghelper.instances[input_id]=self;self.$container=self.$input.closest('div[id^="'+self.prefix+'_input"]');if(PHP.get('ng_developer_debug')){console.log("Container: ");console.log('div[id^="'+self.prefix+'_input"]');console.log('Exists?');console.log(self.$container.exists());}
self.$entry_template=$($('#'+self.$container.attr('id')+'_'+prefix).html());if(PHP.get('ng_developer_debug')){console.log("Entry template: ");console.log('#'+self.$container.attr('id')+'_'+prefix);console.log(self.$entry_template.exists());}
self.names=[];self.max_length=max_length;self.max_entries=max_entries||false;self.options={};self.init();};ngutils.taghelper.prototype={addToCollection:function(str){var self=this;if(PHP.get('ng_developer_debug')){}
var $tag=self.$entry_template.clone();str=str.substr(0,self.max_length);$('span',$tag).html(str);$('input',$tag).val(str);self.register($tag);var $collection=$('.tag-collection',self.$container);if($collection.exists()){$collection.append($tag);}
self.onAddEntry($tag);},checkCollection:function(){var self=this;console.log("Checking collection");console.log(self.$container);var $collection=self.getCollection();ngutils.event.dispatch(ngutils.event.tagsUpdated,this);if($collection&&$collection.exists()){var total=$('div.tag',$collection).length;if(total===0){$collection.hide();}else{$collection.show();}
var $used=$('#'+self.$container.attr('id')+'_used');$used.html(total);if(PHP.get('ng_developer_debug')){console.log("Got "+total+" div.tag elements in collection.");}}else if(PHP.get('ng_developer_debug')){console.log("Collection doesn't exist for checkCollection()");}},pushToCollection:function(tags){var self=this;var existing=$('div',self.getCollection()).length;var found=false;var str;var strip=function(t){return $.strip_text(t,!self.is_user_type);};if(PHP.get('ng_developer_debug')){console.log('pushing tags to collection:');console.log(tags);}
for(var i=0,len=tags.length;i<len;++i){if(self.options&&self.options.strip!==false){str=strip(tags[i]);}else{str=tags[i];}
if(str.length&&!$.rInArray(str.toLowerCase(),self.names)){if(PHP.get('ng_developer_debug')){console.log("calling self.addToCollection");}
self.addToCollection(str);found=true;existing=existing+1;}else if(str.length&&$.rInArray(str.toLowerCase(),self.names)){found=true;}
if(false!==self.max_entries&&existing===self.max_entries){break;}}
self.checkCollection();return found;},disable:function(){var self=this;self.$input.disable();},enable:function(){var self=this;self.$input.enable();$(document).focus();self.init();},getCollection:function(){var self=this;return $('.tag-collection',self.$container);},init:function(){var self=this;if(null===self.max_entries){throw"Please set max entries.";}
...
if(false===self.max_entries||$('div',self.getCollection()).length<self.max_entries){$(this).focus();}else{$(this).prop('disabled',true);self.tabProgressesTo();}};self.$input.off().on('blur',addToCollection).on('paste',function(e){var that=this;setTimeout($.proxy(addToCollection,that),0);}).on('keydown',function(e){var that=this;if(e.which===13){setTimeout(()=>{if(that.value.length){$(that).blur();that.focus();}},0);e.preventDefault();}else if(e.which&&e.which===9){if($(this).data('tablock')){$(this).data('tablock',false);setTimeout(function(){if($(that).prop('disabled')){self.tabProgressesTo();}else{$(that).focus();}},0);}else if(that.value.length){$(this).blur();return false;}else{self.tabProgressesTo();}}});},onAddEntry:function($element){console.debug("taghelper.onAddEntry must be overwritten.");},onRemoveEntry:function($element,editing){console.debug("taghelper.onAddEntry must be overwritten.");},pushName:function(n){this.names.push(n.toLowerCase());},register:function($element){var self=this;if($.isString($element)){$element=$('#'+$element);}
if(!$element.exists()){if(PHP.get('ng_developer_debug')){console.warn("Can't register element, it doesn't exist.");console.log($element);}
return false;}
self.names.push($('span',$element).html().trim());$element.click(function(e){if($(e.target).is('span')){self.removeEntry($element,true);}});$('a[class$="close"]',$element).click(function(){self.removeEntry($(this));});},removeEntry:function($element,push_to_input){var self=this;push_to_input=push_to_input||false;if($element.is('a')){$element=$element.closest('div');}
...
console.log('removeEntry()');console.log(self.$container.html());var $clone=$element;$element.remove();self.onRemoveEntry($clone,push_to_input);self.checkCollection();},tabProgressesTo:function(){setTimeout(function(){$('div[contenteditable]').first().focus();},0);},setStrip:function(bool){this.options.strip=bool;return this;}};ngutils.taghelper.prototype.constructor=ngutils.taghelper;ngutils.taghelper.instances={};ngutils.taghelper.getInstance=function(id){if($.type(id)!=='string'){id=id.attr('id');}
if(ngutils.taghelper.instances[id]){return ngutils.taghelper.instances[id];}else{return null;}};(async($)=>{const enablePod=function(supportObj){$(this).addClass('open');$('input',$(this)).filter('[data-subscription-only!="1"]').enable();let $buttons=$('button',supportObj.getForm());$buttons.enable();};const registerClick=function(supportObj,e){if($(this).hasClass('open')){return;}
const $form=supportObj.getForm();const realclick=(false===e?false:true);const $pods=supportObj.getPurchasePod(true).not($(this));$pods.removeClass('open');$pods.find('input').disable();$pods.find('button').hide();enablePod.call(this,supportObj);const helper=null!==supportObj.purchasee?supportObj.purchasee.getHelper():null;const $user=$('input[name="user"]',$form);if(null!==helper&&!$user.val().length){helper.$input.attr('required','required');}else{if(null!==helper){helper.$input.removeAttr('required');if(true===PHP.get('user_logged_in')){$pods.find('button').show();}}else{$pods.find('button').show();}
if(realclick){let $buttons=$('button',$form);$buttons.disable();supportObj.saveChanges(()=>{$buttons.enable();});}}
supportObj.moveStripeToPod($(this));if(PHP.get('ismobile')&&realclick){$(this).scrollTo();}
return supportObj;};const formatNumberForPrices=(val)=>{return new Intl.NumberFormat('en-US',{style:'currency',currency:'USD'}).format(val);};const metaCaller=function(){this.names={};this.default_prices={};this.contribution_texts={};this.short_descriptions={};this.types={};};metaCaller.prototype={setNames:function(param,names){this.names[param]=names;return this;},setDefaultPrice:function(param,amount){this.default_prices[param]=amount;return this;},setContributionTexts:function(param,contribution_texts){this.contribution_texts[param]=contribution_texts;return this;},setShortDescriptions:function(param,short_descriptions){this.short_descriptions[param]=short_descriptions;return this;},setTypes:function(param,types){this.types[param]=types;return this;},};metaCaller.prototype.constructor=metaCaller;const supporter=function(_id){this._id=_id;this.paymentRequest=null;this.meta=new metaCaller();this.has_purchase_options=false;this.purchasee=null;};supporter.prototype={getContainer:function(){return $(`#${this._id}`);},getForm:function(){let self=this;return $('form',self.getContainer());},getFormData:function(){const unindexed_array=this.getForm().serializeArray();const indexed_array={};$.map(unindexed_array,function(n){indexed_array[n.name]=n.value;});return indexed_array;},getRecurCheckboxes:function(){return this.getForm().find('input[type="checkbox"][name="recur"]');},getPurchasePod:function(param){let self=this;if(undefined===param){return $('div.supporter-purchase.open',self.getForm()).first();}
if(true===param){return $('div.supporter-purchase',self.getForm());}
return $(`div.supporter-purchase[data-param="${param}"]`,self.getForm());},saveChanges:function(callback){if(true!==PHP.get('user_logged_in')){return;}
let self=this;const $f=self.getForm();const serialized=$f.serializeArray();$.post($f.attr('action'),serialized,callback).fail(()=>{$('button',$f).enable();});},setClickables:function(){let self=this;let $all_pods=self.getPurchasePod(true);if($all_pods.toArray().filter((el)=>{return $(el).hasClass('open');}).length===0){enablePod.call($all_pods.first().get(0),self);}
$all_pods.off().on('click',function(e){if(!$(e.target).is(':radio')&&!$(e.target).is('a')){registerClick.call(this,self,e);}});let $pod=self.getPurchasePod();if(undefined!==$pod){self.moveStripeToPod($pod);}},init:function(){let self=this;self.initStripe().initializePurchasables();return self;},initializePurchasables:async function(){let self=this;await(async()=>{self.getPurchasePod(true).each(async function(){await ngutils.event.dispatch(`ngSupporter-${$(this).attr('data-param')}`,self);});})();self.setClickables();if(false===self.has_purchase_options){$('p.supporter-comment',self.getForm()).first().hide();}
const toggleButtons=(enabled)=>{console.log('toggline');const $buttons=$('button',self.getForm());$buttons.prop('disabled',!enabled);const $stripe_el=$('[data-stripe="payment-request-button"]',self.getForm());if(enabled){$stripe_el.show();}else{$stripe_el.hide();}};$('button',self.getForm()).off().click(function(){const $submit=$('button',self.getForm());let callback;const enable=()=>{toggleButtons(true);};if($(this).hasClass('credit-card')){callback=function(){loadCreditCards(enable);};}else{callback=function(){window.onunload=enable;window.location.href='/supporter/payment';};}
toggleButtons(false);self.saveChanges(callback);return false;});self.getRecurCheckboxes().off().click(function(){const $container=$(this).closest('[data-param]');const param=$container.attr('data-param');const index=this.checked?1:0;$('h4',$container).html(self.meta.names[param][index]);$('h4',$container).next('span').html(self.meta.short_descriptions[param][index]);$('input[name="type"]',$container).val(self.meta.types[param][index]);$('strong.contribution_text',$container).html(self.meta.contribution_texts[param][index]);self.updateStripeTotal($container);self.saveChanges();});let last_valid='';const updateContribution=function(){const $pod=$(this).up('[data-param]');let contribution=parseFloat(this.value.replace(/[^0-9.]/,''));if(contribution>10000){contribution=last_valid;}
last_valid=contribution;toggleButtons(true);$('[data-price="contribution"]',$pod).html(formatNumberForPrices(contribution));$('[data-price="user-amount"]',$pod).val(contribution.toFixed(2));$('input[name="amount"]',$pod).val(contribution.toFixed(2));self.updateStripeTotal($pod);};$('input[type="radio"]',self.getForm()).off().click(function(){const $pod=$(this).up('[data-param]');$('[data-price="user-amount"]',$pod).val(this.value);updateContribution.call(this);});$('[data-price="options"] a',self.getForm()).off().click(function(e){e.preventDefault();const $pod=$(this).up('[data-param]');const $parent=$('[data-price="options"]',$pod);$parent.hide();toggleButtons(false);$('[data-price="user-options"]',$pod).show();const $uprice=$('[data-price="user-amount"]',$pod);$uprice.focus();updateContribution.call($uprice[0]);return false;});$('[data-price="user-options"] a',self.getForm()).click(function(e){e.preventDefault();const $pod=$(this).up('[data-param]');const $parent=$('[data-price="user-options"]',$pod);$parent.hide();toggleButtons(false);const $options=$('[data-price="options"]',$pod);$options.show();let $checked=$('input[type="radio"]:checked',$options);if($checked.length<1){$checked=$('input[type="radio"]',$options).first().prop('checked',true);}
updateContribution.call($checked[0]);self.saveChanges();return false;});$('[data-price="user-options"] input[type="text"]',self.getForm()).on('blur',function(e){const $el=$(this);const $container=$el.closest('[data-price="user-options"]');const ohNo=function(){const $closest=$el.closest('[data-price="options"]');const $options=$closest.find('input');$closest.show();$container.hide();$options[0].checked=true;updateContribution.call($options[0]);};if(!this.value.length){if(e.type==='blur'){ohNo.call(this);}}else{const parsed=parseFloat(this.value.replace(/[^0-9.]/,''));const min=parseFloat($(this).attr('data-min-price'));if(isNaN(parsed)){this.value=last_valid;return false;}else if(parsed<min){return false;}else{if(this.value!==$(this).data('contribution')){$(this).data('contribution',this.value);updateContribution.call(this);}}}});$('input',self.getForm()).change(function(){if(this.name.length&&this.name==='user_purchase'){return;}});self.saveChanges();window.onunload=self.saveChanges.bind(self);return self;},initPurchasee:function(config){config=typeof(config)!==typeof(undefined)?config:{};if(!config.hasOwnProperty('form')){config.form=this.getForm();}
config.supporter=this;this.purchasee=new purchasee(config);this.purchasee.initPurchaser();return this;},initStripe:function(){let self=this;const stripe=Stripe(PHP.req('stripe_pk'));const reportPaymentRequestError=(error)=>{let $el=$('div[data-stripe="payment-request-errors"]',self.getForm());if(error){$('span',$el).text(error);$el.show();}else{$el.hide();}};const paymentRequest=stripe.paymentRequest({currency:'usd',country:'US',requestPayerName:true,requestPayerEmail:true,total:{label:'Arbitrary amount.',amount:100,}});const elements=stripe.elements();const prButton=elements.create('paymentRequestButton',{paymentRequest:paymentRequest,style:{paymentRequestButton:{height:PHP.get('ismobile')?'70px':'40px',},},});paymentRequest.canMakePayment().then((result)=>{let $el=$('[data-stripe="payment-request-button"]',self.getForm());if(result&&undefined!==$el.get(0)){prButton.mount($el.get(0));}else{$el.hide();$('.stripe-holder',self.getForm()).slideUp('fast',(el)=>$(el).remove());}});paymentRequest.on('paymentmethod',async(e)=>{const $f=self.getForm();const form_data=new FormData($f.get(0));form_data.append('email',(typeof(e.payerEmail)!==typeof(undefined)?e.payerEmail:null));form_data.append('walletName',e.walletName);reportPaymentRequestError(false);const{client_secret}=await fetch('/supporter/intent',{method:'POST',body:form_data,}).then(r=>r.json());const{error,paymentIntent}=await stripe.confirmCardPayment(client_secret,{payment_method:e.paymentMethod.id,},{handleActions:false});if(error){reportPaymentRequestError(error);e.complete('fail');return;}
e.complete('success');if(paymentIntent.status==='requires_action'){const{error,paymentIntent}=await stripe.confirmCardPayment(client_secret);if(error){reportPaymentRequestError(error);return;}}
$.get('/supporter/intent?success',(response)=>{if(PHP.get('ismobile')){let $el=$('#body-main');$('.column',$el).remove();$el.append(response);}else{$('.body-guts').html(response);}});});self.paymentRequest=paymentRequest;return self;},getStripeTotal:function($pod){let self=this;$pod=undefined!==$pod?$pod:self.getPurchasePod();const meta=self.meta;const param=$pod.attr('data-param');const $recur=$(`input[name="recur"]`,$pod);const recurring=+($recur.exists()&&$recur.is(':checked'));return{label:meta.names[param][recurring],amount:parseInt($(`input[name="amount"]`,$pod).val()*100,10),};},updateStripeTotal:function($pod){let self=this;$pod=undefined!==$pod?$pod:self.getPurchasePod();const total=self.getStripeTotal($pod);self.paymentRequest.update({total:total,});return self;},moveStripeToPod:function($pod){let self=this;const $el=$('[data-stripe="payment-request"]',self.getContainer()).detach();const $destination=$('div[data-stripe-destination]',$pod);if(!$destination.exists()){$el.remove();return;}
$destination.append($el);return self.updateStripeTotal($pod);},};supporter.prototype.constructor=supporter;const purchasee=function(config){this.supportObj=config.supporter;this.helper=null;this.$form=(typeof(config.form)!==typeof(undefined)?config.form:null);this.$suggest_input=(typeof(config.input)!==typeof(undefined)?config.input:null);};const loadCreditCards=(callback)=>{const onFail=function(jqXHR){console.dir(jqXHR);if(jqXHR.status===302){alert("Looks like you may have logged out? You must be logged in to purchase anything!");}else{alert("Sorry, that request failed!");}
this.hide();};const bo=new ngutils.blackout({remove_on_hide:true,remove_on_click_outside:false,_onfail:onFail});bo.load('/supporter/credit-card',null,callback);return false;};purchasee.prototype={getHelper:function(){return this.helper;},initPurchaser:function(){let self=this;const $form=self.supportObj.getForm();const $user=$('input[type="hidden"][name="user"]',$form);const helper=ngutils.taghelper.getInstance($('input[name="user_purchase"]',$form));helper.onAddEntry=function($tag){helper.$container.hide();$.post('/supporter/purchase-options',{'user':$('input[type="hidden"]',$tag).val()},function(response){$form.parent('div').replaceWith(response);let supportObj=self.supportObj;}).fail(function(){helper.$container.show();});};const $purchase_for_container=$('[data-container="purchase-for"]',$form);$('a[class^="ngicon-"]',$purchase_for_container).off().click(function(){$purchase_for_container.hide();self.supportObj.getPurchasePod(true).hide();$('p.supporter-comment',$form).hide();$('[data-container="gifting"]',$form).hide();helper.$container.show();});if($user.val().length){helper.$container.hide();}}};purchasee.prototype.constructor=purchasee;ngutils.supporter=supporter;})(jQuery);ngutils.uploadEvent=function(event,uploader,target,data){this.event=event;this.uploader=uploader;this.target=target;this.data=data;};ngutils.uploadEvent.onSubmit="ngutils.uploadEvent.onSubmit";ngutils.uploadEvent.onUpload="ngutils.uploadEvent.onUpload";ngutils.uploadEvent.onCancel="ngutils.uploadEvent.onCancel";ngutils.uploadEvent.onDeleted="ngutils.uploadEvent.onDeleted";ngutils.uploadEvent.onError="ngutils.uploadEvent.onError";ngutils.uploadEvent.onProgress="ngutils.uploadEvent.onProgress";ngutils.uploader=function(element_selector,endpoint,config){var _uploader=this;var webImageTypes=['gif','jpg','jpeg','png','webp'];if(!config)config={};var debug=config.debug?true:false;var multi=typeof(config.multiple)===typeof(undefined)?false:(config.multiple?true:false);var chunking=typeof(config.chunking)===typeof(undefined)?true:(config.chunking?true:false);var chunkSize=config.chunkSize?config.chunkSize:5000000;var maxConnections=config.maxConnections?config.maxConnections:3;var allowedExtensions=config.allowedExtensions?config.allowedExtensions:webImageTypes;var template=config.template?config.template:"upload_template_default";var sizeLimit=config.sizeLimit?config.sizeLimit:2e+9;var redirectsEnabled=config.redirectsEnabled?config.redirectsEnabled:true;_uploader.onPreviewImage=config.onPreviewImage?config.onPreviewImage:null;_uploader.onSelect=config.onSelect?config.onSelect:null;_uploader.onError=config.onError?config.onError:null;_uploader.onComplete=config.onComplete?config.onComplete:null;_uploader.onCancel=config.onCancel?config.onCancel:null;var liveUpdate=config.liveUpdate?config.liveUpdate:false;var $uploader=$(element_selector);this.id=config.id?config.id:$uploader.attr('id');this.useBlackout=config.useBlackout?config.useBlackout:false;this.preserveBlackout=config.preserveBlackout?config.preserveBlackout:false;var $droparea,$drop_area_text,$uploader_inner,$preview_img;let hideDroparea=()=>{$droparea.hide();};let showDroparea=()=>{$droparea.show();};var minImageWidth=typeof(config.minImageWidth)=='number'?config.minImageWidth:null;var maxImageWidth=typeof(config.maxImageWidth)=='number'?config.maxImageWidth:null;var minImageHeight=typeof(config.minImageHeight)=='number'?config.minImageHeight:null;var maxImageHeight=typeof(config.maxImageHeight)=='number'?config.maxImageHeight:null;var maxGIFHeight=typeof(config.maxGIFHeight)=='number'?config.maxGIFHeight:maxImageHeight;var maxGIFWidth=typeof(config.maxGIFWidth)=='number'?config.maxGIFWidth:maxImageWidth;const blackout=new ngutils.blackout({remove_on_click_outside:false});this.removeBlackout=function(){blackout.hide();};this.removeBlackoutAuto=function(){if(true!==this.preserveBlackout){this.removeBlackout();}};var fuconfig={messages:{},maxConnections:maxConnections,template:template,element:$uploader[0],multiple:multi,text:{fileInputTitle:" "},request:{endpoint:endpoint},chunking:{enabled:chunking},callbacks:{onSubmit:function(id,name){var _fu=this;var is_gif=name.split(".").pop().toLowerCase()=='gif';return new Promise((proceed,stop)=>{var event=new ngutils.uploadEvent(ngutils.uploadEvent.onSubmit,_uploader,null,{id:id,name:name});ngutils.event.dispatch(event.event,event);function checkOnSelect(){if(typeof(_uploader.onSelect)=='function'){let success=_uploader.onSelect.call(_fu,id,name);if(typeof(success)==='boolean'){if(success){proceed();}else{stop();}}else{success.then(proceed).catch(stop);}}else{proceed();}}
...
if(!multi)showDroparea();var event=new ngutils.uploadEvent(ngutils.uploadEvent.onComplete,_uploader,$file,{id:id,name:name,response:response});ngutils.event.dispatch(event.event,event);if(typeof(_uploader.onComplete)=='function'){_uploader.onComplete.call(this,id,name,response);}},onCancel:function(id,name){_uploader.removeBlackoutAuto();$uploader_inner.removeClass('uploading');if(!multi)showDroparea();var $file=$(this.getItemByFileId(id));var event=new ngutils.uploadEvent(ngutils.uploadEvent.onCancel,_uploader,$file,{id:id,name:name});ngutils.event.dispatch(event.event,event);if(typeof(_uploader.onCancel)=='function')return _uploader.onCancel.call(this,id,name);},onError:function(id,name,errorMsg){_uploader.removeBlackoutAuto();var $file=$(this.getItemByFileId(id));if($uploader_inner&&$uploader_inner.exists()){$uploader_inner.removeClass('uploading');}
if(!multi){showDroparea();}
var $error=$(".qq-error",$(this.getItemByFileId(id))).first();if(typeof(_uploader.onError)==='function'){_uploader.onError.call(this,id,name,errorMsg);}else if($error.length>0){$error.text(errorMsg);}else{alert(errorMsg);}
...
return null;};};ngutils.uploads={deleteImageUpload:function(url,source_type,source_id,callback){$.post('/image-delete',{url:url,source_type:source_type,source_id:source_id,userkey:PHP.get('uek'),},callback,'json').fail((x)=>{if(x.status!==403){alert('Unexpected error!');console.error('Unexpected error',x.responseText);}});}};ngutils.croptool=function(config){var croptool=this;var id=config.id;var saving=false;var dragging=false;var mousePos={x:0,y:0};var imgLoader=new Image();var width=config.width;var height=config.height;var anchor,minscale,maxscale,scale,scalerange;var maxzoom=config.maxZoom?config.maxZoom:'auto';var $body=$('body').first();var $template=$('#'+id);var $ui=$('<div>');$body.append($ui);$ui.html($template.html());$template.remove();var $frame=$('[data-crop-frame]',$ui).first();var $image=$('[data-cropping-image]',$ui).first();var $save_btn=$('[data-btn-save]',$ui).first();var $cancel_btn=$('[data-btn-cancel]',$ui).first();var $save_txt=$('[data-saving-text]',$ui).first();var $slider_bar=$('[data-scale-slider-container]',$ui).first();var $shape_select=$('[data-shape-select]',$ui).first();var $shape_frame=$('[data-shape-frame]',$ui).first();var $slider=$('[data-scale-slider]',$ui).first();var $slider_buttons=$('[data-increment-slider]',$ui.first());$ui.css('display','none');$image.css('position','absolute');this.parked_id=null;this.original_src=null;this.original_crop=null;this.shape=config.shape;this.last_shape=config.shape;this.last_crop={x:0,y:0,width:width,height:height};this.user=null;this.user_id=null;ngutils.croptool.ids[id]=this;this.startSave=function(){saving=true;$save_btn.disable();$cancel_btn.disable();$save_txt.show();$slider_bar.hide();return this;};this.endSave=function(){saving=false;$save_btn.enable();$cancel_btn.enable();$save_txt.hide();$slider_bar.show();return this;};this.reset=function(){anchor={x:0,y:0};minscale=1;maxscale=2;maxzoom='auto';scale=1;scalerange=1;$image.removeAttr('src');$(imgLoader).removeAttr('src');$slider.slider('value',0);return this;};this.onCancel=function(){};this.onSave=function(crop,frame){};this.afterSave=function(response){};this.hide=function(){$body.css('overflow','');$ui.css('display','none');ngutils.croptool.current=null;return this;};this.show=function(){if(ngutils.croptool.current)ngutils.croptool.current.hide();ngutils.croptool.current=this;$body.css('overflow','hidden');$ui.css('display','');return this;};this.update=function(){let dimensions={width:imgLoader.width*scale,height:imgLoader.height*scale,left:Math.round((width / 2)-(anchor.x*scale)),top:Math.round((height / 2)-(anchor.y*scale)),};$image.css(dimensions);return this;};var imageLoaded=function(){let wscale=width / imgLoader.width;let hscale=height / imgLoader.height;let _maxzoom=maxzoom;if(maxzoom==='auto'){let xz=(imgLoader.width / width);let yz=(imgLoader.height / height);_maxzoom=xz>yz?xz:yz;}
minscale=wscale>hscale?wscale:hscale;maxscale=minscale*_maxzoom;scale=minscale;scalerange=maxscale-minscale;anchor.x=Math.floor(imgLoader.width / 2);anchor.y=Math.floor(imgLoader.height / 2);let info={minscale:minscale,maxscale:maxscale,scale:scale,scalerange:scalerange,anchor:anchor,};console.table(info);let $imp=$('[data-scale-impossible]',$ui);if(!scalerange){$slider.slider('disable');$slider_buttons.addClass('disabled');$imp.show();}else{$slider.slider('enable');$slider_buttons.removeClass('disabled');$imp.hide();}
croptool.update();};this.checkAnchor=function(){var fh=(height / 2)/ scale;var fw=(width / 2)/ scale;if(anchor.y<fh)anchor.y=fh;else if(anchor.y>imgLoader.height-fh)anchor.y=imgLoader.height-fh;if(anchor.x<fw)anchor.x=fw;else if(anchor.x>imgLoader.width-fw)anchor.x=imgLoader.width-fw;return this;};this.onDragged=function(moved){anchor.x-=moved.x / scale;anchor.y-=moved.y / scale;this.checkAnchor();return this.update();};this.setCrop=function(crop){console.log(`setCrop(crop);`);console.log(crop);let wr=width / crop.width;let hr=height / crop.height;scale=width / crop.width;var fw=(width / 2)/ scale;var fh=(height / 2)/ scale;let scaleVal=(scale-minscale)/ scalerange;console.log(`scale is ${scale}, minscale is ${minscale}, scalerange is ${scalerange}, scaleVal is ${scaleVal}`);$slider.slider('value',isNaN(scaleVal)?0:scaleVal);anchor.x=crop.x+fw;anchor.y=crop.y+fh;croptool.update();return this;};this.setOriginalCrop=function(crop){if(null===this.original_crop){this.original_crop=crop;}
return this;};this.setOriginalSrc=function(src){if(null===this.original_src){this.original_src=src;}
return this;};this.setParkedId=function(parked_id){this.parked_id=parked_id;return this;};this.setSrc=function(src){return new Promise(function(resolve,reject){$image.removeAttr('src');imgLoader=new Image();imgLoader.onload=()=>{imageLoaded();resolve(`image loaded: ${src}`);};$image.attr('src',src);imgLoader.src=src;});};this.setUserParams=function(params){this.user_id=params.user_id;this.user=params.user;return this;};this.save=function(crop){const commonAfterSave=()=>{croptool.endSave();croptool.hide();};let params={editing:null!==this.parked_id,userkey:PHP.get('uek'),type:'userpic',referrer:'//'+window.location.host+window.location.pathname,user:this.user,id:this.user_id,thumb_crop_width:crop.width,thumb_crop_height:crop.height,thumb_top_y:crop.top,thumb_top_x:crop.left,parked_id:this.parked_id,shape:this.last_shape,};$.post('/icons/submit',params,function(r){commonAfterSave();croptool.setOriginalSrc(r.raw).setOriginalCrop(crop).setParkedId(null);croptool.afterSave(r);},'JSON').fail(commonAfterSave);};this.updateFromExisting=function(src,cropinfo,callback){croptool.setSrc(src).then(function(){croptool.setOriginalSrc(src).setOriginalCrop(cropinfo).setCrop(cropinfo).setShape(croptool.last_shape).checkAnchor();if(typeof callback==='function'){callback(croptool);}});return this;};this.updateFromParked=function(response){let croptool=this;return new Promise((resolve,reject)=>{this.reset().setParkedId(response.parked_id).setShape(croptool.last_shape).setSrc(response.parked_url).then(function(){croptool.update().show();});resolve(`loaded ${response.parked_url}`);});};const getSliderButtonVal=(button,nullify)=>{const $button=$(button);nullify=nullify||false;const cur=$slider.slider('option','value');let val;if($button.data('incrementSlider')==='zoom-in'){val=cur+0.2;}else{val=cur-0.2;}
if(val>1)val=nullify?null:1;if(val<0)val=nullify?null:0;return val;};const doSliderButtonCursor=function(){if(null!=getSliderButtonVal(this,true)&&!$slider.slider('option','disabled')){$(this).css('cursor',$(this).data('incrementSlider'));}else{$(this).css('cursor','default');}};const scaleMe=(val)=>{scale=minscale+(scalerange*val);croptool.checkAnchor();croptool.update();};$slider.slider({value:0,min:0,max:1,step:0.01,slide:function(event,ui){scaleMe(ui.value);}});$slider_buttons.mouseover(doSliderButtonCursor).click(function(){if($slider.slider('option','disabled')){return;}
let val=getSliderButtonVal(this);$slider.slider('option','value',val);scaleMe(val);doSliderButtonCursor.call(this);}).mouseout(doSliderButtonCursor);var $shape_btns=$('[data-shape-id]',$shape_select);this.setShape=function(id){if(!id)id=0;croptool.shape=parseInt(id);$shape_btns.removeClass('selected');$('[data-shape-id='+croptool.shape+']',$shape_select).addClass('selected');$shape_frame.attr('href',"#path_s"+croptool.shape);return this;};$shape_btns.click(function(e){e.preventDefault();croptool.setShape(parseInt($(this).attr('data-shape-id')));});$cancel_btn.click(function(){const commonCancel=()=>{croptool.hide();croptool.onCancel();};if(null!==croptool.original_src){if(null!==croptool.parked_id){croptool.setParkedId(null);}
croptool.setSrc(croptool.original_src).then(()=>{if(null!==croptool.original_crop){croptool.setCrop(croptool.original_crop);}
commonCancel();});}else{commonCancel();}});$save_btn.click(function(){croptool.startSave();var crop={};var fh=(height / 2)/ scale;var fw=(width / 2)/ scale;crop.top=Math.ceil(anchor.y-fh);crop.left=Math.ceil(anchor.x-fw);crop.width=Math.floor(fw*2);crop.height=Math.floor(fh*2);crop.right=crop.left+crop.width;crop.bottom=crop.top+crop.height;croptool.last_shape=croptool.shape;croptool.last_crop={x:crop.top,y:crop.left,width:crop.width,height:crop.height};croptool.save(crop);});$image.on(ngutils.input.mouseDown,function(e){dragging=true;var posObj=e.changedTouches?e.changedTouches[0]:e;mousePos={x:posObj.screenX,y:posObj.screenY};});$(document).on(ngutils.input.mouseUp,function(){dragging=false;}).on(ngutils.input.mouseMove,function(e){if(!dragging||saving)return;var posObj=e.changedTouches?e.changedTouches[0]:e;var newPos={x:posObj.screenX,y:posObj.screenY};if(mousePos){croptool.onDragged({x:newPos.x-mousePos.x,y:newPos.y-mousePos.y});}
mousePos=newPos;return false;});if(config.src){imgLoader.src=config.src;}
this.reset();};ngutils.croptool.current=null;ngutils.croptool.ids={};ngutils.croptool.get=function(id){return typeof(ngutils.croptool.ids[id])!='undefined'?ngutils.croptool.ids[id]:null;};ngutils.croptool.prototype.constructor=ngutils.croptool;ngutils.croppable_image=function(selector,config){let _this=this;this.onSaved=function(result){};this.onCancel=function(){};this.parked_id=null;this.parked_url='';var skip_crop=false;var image_crop=this;var image=this.image=new Image();var $container=$(selector);var $aimage=$('[data-image-frame]',$container).first();var $img=$('img',$aimage).first();var $spinner=$('[data-image-spinner]',$container).first();var $save_text=$('[data-saving-text]',$container).first();var $upload=$('div[data-image-upload]',$container).first();var $upload_container=$('div[data-image-upload-toggle]',$container).first();var alwaysRedraw=config.alwaysRedraw?config.alwaysRedraw:false;var parkEndpoint=config.parkEndpoint?config.parkEndpoint:"/parkfile";var saveEndpoint=config.saveEndpoint?config.saveEndpoint:null;var liveCrop=config.liveCrop?config.liveCrop:false;var saveParams=config.saveParams?config.saveParams:null;var stretchToFit=typeof(config.stretchToFit)!==typeof(undefined)?config.stretchToFit:true;var fileTypesText=config.fileTypesText?config.fileTypesText:null;var dropAreaText=config.dropAreaText?config.dropAreaText:"Drop Image, or Click Here";var unstretchImage=function(){if(stretchToFit)return;setTimeout(function(){if($img[0].naturalWidth<$img[0].width||$img[0].naturalHeight<$img[0].height){$img[0].width=$img[0].naturalWidth;$img[0].height=$img[0].naturalHeight;$img.removeAttr('style');}},100);};$img[0].onload=unstretchImage;var canEdit=$upload.length>0;var $crop_ui=$('[data-crop-ui]',$container).first();if(!canEdit){$crop_ui.remove();}else{$crop_ui.addClass('nodim');}
...
this.maxFrameWidth=config.frame.maxWidth?config.frame.maxWidth:1080;this.maxFrameHeight=config.frame.maxHeight?config.frame.maxHeight:360;this.minFrameWidth=config.frame.minWidth?config.frame.minWidth:480;this.minFrameHeight=config.frame.minHeight?config.frame.minHeight:240;this.fillScale=config.fillScale?config.fillScale:1;this.keepAspect=config.frame.keepAspect?config.frame.keepAspect:false;if(canEdit){var uconfig={template:"croppable_image_template",fileTypesText:fileTypesText,dropAreaText:dropAreaText,multiple:false,allowedExtensions:["webp","png","jpeg","jpg","gif",],onComplete:function(id,filename,response){if(typeof(id)=='number')$(this.getItemByFileId(id)).remove();if(skip_crop){startSaving();var output={nocrop:true,parked_id:response.parked_id,userkey:PHP.get('uek'),};for(var i in saveParams){output[i]=saveParams[i];}
$.post(saveEndpoint,output,function(result){image_crop.oldSrc=result.image_url;image_crop.clearEdit();image_crop.onSaved(result);},'json').fail(function(){image_crop.clearEdit();endSaving();});}else{image_crop.parked_id=response.parked_id;image_crop.parked_url=response.parked_url;image_crop.newImage(image_crop.parked_url);}}};if(config.acceptFiles)uconfig.acceptFiles=config.acceptFiles;if(config.allowedExtensions)uconfig.allowedExtensions=config.allowedExtensions;if(config.sizeLimit)uconfig.sizeLimit=config.sizeLimit;if(config.maxWidth)uconfig.maxImageWidth=config.maxWidth;if(config.maxHeight)uconfig.maxImageHeight=config.maxHeight;if(config.minWidth)uconfig.minImageWidth=config.minWidth;if(config.minHeight)uconfig.minImageHeight=config.minHeight;uconfig.maxGIFWidth=this.maxFrameWidth;uconfig.maxGIFHeight=this.maxFrameHeight;if(config.uconfig){for(let n in config.uconfig){uconfig[n]=config.uconfig[n];delete config.uconfig[n];}
delete config.uconfig;}
if(config.debug)uconfig.debug=true;if(liveCrop){uconfig.autoUpload=false;uconfig.onPreviewImage=function($thumb,id,name){let _croppable_image=this;return new Promise((proceed,reject)=>{if(false===alwaysRedraw&&$thumb[0].naturalWidth<=image_crop.maxFrameWidth&&$thumb[0].naturalHeight<=image_crop.maxFrameHeight){$thumb=null;skip_crop=true;return proceed();}
skip_crop=false;var complete=function(){reject();image.width=$img[0].naturalWidth;image.height=$img[0].naturalHeight;image.src=$img.attr('src');uconfig.onComplete.call(_croppable_image,null,null,{parked_id:0,parked_url:image.src});};var drawToCanvas=(canvas)=>{$thumb=null;let $tmp=$("<img>");$tmp[0].onload=function(){$img[0].src=$tmp[0].src;complete();};$tmp.attr('src',canvas.toDataURL('image/webp',0.9));};if($thumb[0].naturalWidth<=image_crop.maxFrameWidth&&$thumb[0].naturalHeight<=image_crop.maxFrameHeight){let canvas=document.createElement('canvas');let context2d=canvas.getContext('2d');canvas.width=image_crop.maxFrameWidth;canvas.height=image_crop.maxFrameHeight;context2d.drawImage($thumb[0],Math.round((canvas.width-$thumb[0].naturalWidth)/ 2),Math.round((canvas.height-$thumb[0].naturalHeight)/ 2),$thumb[0].naturalWidth,$thumb[0].naturalHeight);drawToCanvas(canvas);}else if($thumb[0].naturalWidth<image_crop.maxFrameWidth){let canvas=document.createElement('canvas');let context2d=canvas.getContext('2d');canvas.width=image_crop.maxFrameWidth;canvas.height=$thumb[0].naturalHeight;context2d.drawImage($thumb[0],Math.round((canvas.width-$thumb[0].naturalWidth)/ 2),0,$thumb[0].naturalWidth,$thumb[0].naturalHeight);drawToCanvas(canvas);}else{$img[0].src=$thumb[0].src;complete();}});};}
const uploader=new ngutils.uploader($upload,parkEndpoint,uconfig);setTimeout(unstretchImage,300);}
this.dragMove=null;var first_load=true;var updateSrc=function(src){if(undefined===src)return;if(!first_load&&'#'!==src){$spinner.show();}else{$spinner.hide();}
first_load=false;image.src=src;};var mode='view';var dragging=false;mousePos=null;if(canEdit){$aimage.on(ngutils.input.mouseDown,function(e){dragging=mode!=='view'?true:false;var posObj=e.changedTouches?e.changedTouches[0]:e;mousePos={x:posObj.screenX,y:posObj.screenY};});$(document).on(ngutils.input.mouseUp,function(){dragging=false;}).on(ngutils.input.mouseMove,function(e){if(!dragging||saving)return;var posObj=e.changedTouches?e.changedTouches[0]:e;var newPos={x:posObj.screenX,y:posObj.screenY};if(mousePos){image_crop.onDragged({x:newPos.x-mousePos.x,y:newPos.y-mousePos.y});}
...
this.minScale=1;this.maxScale=2;this.anchor={x:0,y:0};var frame={width:this.maxFrameWidth,height:Math.ceil(this.maxFrameHeight /(this.minFrameWidth/ this.maxFrameWidth))};var $save=$('[data-btn-save]',$crop_ui).first();var $cancel=$('[data-btn-cancel]',$crop_ui).first();var saving=false;var startSaving=function(){saving=true;$cancel.disable();$save.disable();$slider_container.hide();$save_text.show();};var endSaving=function(){saving=false;$cancel.enable();$save.enable();$slider_container.show();$save_text.hide();};endSaving();const handleFiles=(files)=>{if(files&&files.length){const uploadingFile=(file)=>{let task=ngutils.activeTasks.addTask("We're still loading your image, are you sure you want to leave?");let fr=new FileReader();fr.addEventListener('load',()=>{_this.refresh=true;_this.image.src=fr.result;task.completed();});_this.imageSelected(file);fr.readAsDataURL(file);};for(let i=0,len=files.length;i<len;i++){let file=files[i];if(file.type.split("/")[0]==='image'){uploadingFile(file);break;}}}};if(canEdit){$cancel.click(function(){image_crop.cancelEdit();return false;});$save.click(function(){const onError=(err)=>{alert(err);endSaving();return false;};if(liveCrop){let $img=$('img',$aimage).first();let src=$img.attr('src');if(!src){alert('Please upload an image first!');return false;}
...
context2d.drawImage($img[0],$img.position().left*scale,$img.position().top*scale,$img.width()*scale,$img.height()*scale);let base64=canvas.toDataURL('image/webp',0.8).split(",").pop();var blob=ngutils.b64toBlob(base64,'image/webp');var formData=new FormData();formData.append("qquuid","auto");formData.append("qqfile",blob);formData.append("qqfilename",'qqCanvasImage.webp');formData.append("userkey",PHP.get("uek"));if(saveParams){for(let param in saveParams){formData.append(param,saveParams[param]);}}
$.ajax({url:saveEndpoint,type:"POST",data:formData,contentType:false,cache:false,processData:false,dataType:"json",error:onError,success:function(result){image_crop.oldSrc=result.image_url?result.image_url:null;image_crop.clearEdit();image_crop.onSaved(result);}}).fail(onError);endSaving();};if($img.naturalWidth===0){$img[0].onload=function(){doLiveCrop();};}else{doLiveCrop();}}else{startSaving();let output={crop:image_crop.crop,frame:frame,parked_id:image_crop.parked_id,userkey:PHP.get('uek'),};for(let i in saveParams){output[i]=saveParams[i];}
$.post(saveEndpoint,output,function(result){image_crop.oldSrc=result.image_url;image_crop.clearEdit();image_crop.onSaved(result);},'json').fail(function(){endSaving();}).fail(onError);}
return false;});}
this.cancelEdit=function(){image_crop.clearEdit();image_crop.onCancel();};this.clearEdit=function(){ngutils.blackout.undim();$crop_ui.hide();endSaving();mode='view';$container.removeClass('nodim');$container.removeClass('img-edit');$upload_container.addClass('can-upload');if(image_crop.oldSrc){updateSrc(image_crop.oldSrc);}else{updateSrc('#');}};this.newImage=function(url){if(undefined===url){return;}
mode='new';updateSrc(url);$container.addClass('img-edit');let $slider=$('[data-scale-slider]',$crop_ui).first();const scaleAndDraw=(val)=>{image_crop.fillScale=image_crop.minScale+((image_crop.maxScale-image_crop.minScale)*val);image_crop.draw();};$slider.slider({value:0,min:0,max:1,step:0.001,slide:function(event,ui){scaleAndDraw(ui.value);}});$('[data-increment-slider]',$crop_ui).mouseover(function(){$(this).css('cursor',this.dataset.incrementSlider);}).mouseout(function(){$(this).css('cursor','default');}).click(function(){let current=$slider.slider('value');let step=$slider.slider('option','step');let n=current;if('zoom-in'===this.dataset.incrementSlider){n+=0.2;}else{n-=0.2;}
$slider.slider('value',n);scaleAndDraw(n);});$crop_ui.show();$container.addClass('nodim');ngutils.blackout.dim();$upload_container.removeClass('can-upload');};this.onDragged=function(dragged){this.dragMove=dragged;this.draw();};this.crop=null;this.draw=function(){$spinner.hide();if(!image.src||!image.width)return;if($img.attr('src')!==image.src){$img.attr('src',image.src);$img.css('visibility','hidden');}
$img.css("width","");$img.css("height","");$img.css('top',"");$img.css('left',"");if(image_crop.keepAspect){image_crop.updateHeight();}
...
updateTarget(data.user_id,null,data,blocked);for(var page in data.exemptions){var target=page+'-'+data.user_id;blocked=!data.exemptions[page];var uparams=updateTarget(target,page,data,blocked);if(uparams)ngutils.event.dispatch(ngutils.event.userBlockUpdated,uparams);}}};ngutils.event.addListener(ngutils.event.userBlocksUpdated,ngutils.user.block.handleUpdates);ngutils.moderation={enableImageModeration:($img,callback)=>{$img=$($img);let $wrapper=$($img).parent();let $module=$('<div class="padded" style="position:absolute; top:0px; right: 0px"><a class="ngicon-25-trash" title="Delete Image" style="cursor: pointer;"></a></div>');$wrapper.append($module);$('a',$module).click((e)=>{e.preventDefault();if(confirm("Are you sure you want to delete this image?")){callback(()=>{$module.remove();ngutils.element.fancyRemove($img,()=>{$wrapper.remove();});});}});}};ngutils.blogpost={contentType:4001,enableImageModeration:($img)=>{$img=$($img);ngutils.moderation.enableImageModeration($img,(onComplete)=>{let source_id=parseInt($img.closest('[data-post-id]').attr('data-post-id'));ngutils.uploads.deleteImageUpload($img.attr('src'),ngutils.blogpost.contentType,source_id,(result)=>{onComplete();});});},};ngutils.bbspost={contentType:3002,form_id:0,unbanFromClick:(event,button)=>{event.preventDefault();var user_id=$(button).attr('id').split('_').pop();var user=PHP.get('bbs_poster_'+user_id,"this user");if(!confirm("Are you sure you want to clear\n"+user+"'s ban?"))return;$.post(button.href,()=>{let $parent=$(button).closest('[data-ban-details-for]');let $ban_btn=$('[id="ban-btn-text"]',$parent);let $clear_btn=$('[id="clear-ban-btn"]',$parent);let $ban_details=$('[id="ban-details"]',$parent);$clear_btn.hide();$ban_details.hide();$ban_btn.html('ban');});},deleteFromClick:(event,button,post_id,which)=>{event.preventDefault();let $button=$(button);if(confirm('Are you sure you want to delete this '+which+'?')){$.post($button.attr('href'),()=>{let $remove;if(which==='pic'){$button.remove();ngutils.element.fancyRemove($('[id="bbspost_img_'+post_id+'"]').parent());}else{ngutils.element.fancyRemove($('[id="bbspost'+post_id+'_post_text"]').parent());}});}},enableImageModeration:($img)=>{$img=$($img);ngutils.moderation.enableImageModeration($img,(onComplete)=>{let source_id=parseInt($img.closest('[data-post-id]').attr('data-post-id'));ngutils.uploads.deleteImageUpload($img.attr('src'),ngutils.bbspost.contentType,source_id,(result)=>{onComplete();});});},loadFormOnClick:(event,button,post_id,container,context,_confirm)=>{event.preventDefault();if(_confirm){if(!confirm("This is not your post, are you sure you want to edit?"))return false;}
if(!PHP.get('user_logged_in')){PassportHandler.open();return;}
let $button=$(button);ngutils.bbspost.loadForm(post_id,$button.attr('href'),container+"",context);},loadForm:(post_id,href,container,context)=>{ngutils.bbspost.form_id++;const fid=ngutils.bbspost.form_id;const bo=new ngutils.blackout({remove_on_click_outside:false,remove_on_hide:true});if(!context)context='post';let $container=$(container);ngutils.event.addListener('bbs-form-loaded-'+fid,(data)=>{if(!data.result||typeof(data.result.url)===typeof(undefined)){data.posthandler.handled_by_event=true;alert("There was an unexpected problem with your post.");return;}
if($container.length<1||typeof(data.result.html)===typeof(undefined)){bo.hide();data.posthandler.handled_by_event=false;return;}
if(context==='edit'){console.log(fid);data.posthandler.handled_by_event=true;$('[id="bbspost'+post_id+'_post_text"]').up('.column').replaceWith($(data.result.html));bo.setBody("<p><strong>Changes Saved!</strong></p>");setTimeout(()=>{bo.hide();},750);}else{console.log("No context was given here or it was post? "+context);data.posthandler.handled_by_event=true;bo.hide();$container.css('display','block');let $parent=$container.closest(".blackout-hover");let top;if($parent.length<1){$parent=$('html,body');top=$container.offset().top;}else{top=$container.position().top;}
top+=$container.outerHeight();let $post=$('<div>');$post.append($(data.result.html));$post=$('div:first',$post);if(!data.result.noscroll)$parent.stop().animate({scrollTop:top},300);$container.append($post);if($post.width())$post.attr('data-scale-width',$post.width());$post.hide();setTimeout(()=>{$('[data-smart-scale]',$post).each(()=>{new ngutils.media.smartscale($(this));});},1);$post.css('visibility','hidden');setTimeout(()=>{$post.css('visibility','').hide();$post.fadeIn(800,()=>{});},250);}});bo.load(href,{form_id:fid,container_id:$container.attr('id'),from_page:PHP.get('bbs_topic_page',0)});bo._onfail=function(){this.hide();};}};ngutils.event.addListener('bbs-user-banned',(data)=>{let $details=$('[data-ban-details-for="'+(data.user.toLowerCase())+'"]');let $new=$(data.html);$details.html($new.html());});ngutils.bbssig={deleteFromClick:(event,button,user_id,which)=>{event.preventDefault();let $button=$(button);let msg="signature";if(which=='text')msg="sig text";else if(which=='pic')msg="sig banner";if(!confirm("Are you sure you want to delete this "+msg+"?"))return;let $e=$('[id^="bbs_sig_"][id$="_'+user_id+'"]');function deleteText($e){$('.sig-text',$e).remove();$('[id^="delete_sig_text_"]',$e).remove();}
...
if(!$.trim($sig_text.html())&&!$sig_image.attr('src')){if(is_activeuser){$('.add-signature-link',$(this)).show();}else{$(this).hide();}}else{if(is_activeuser){$('.add-signature-link',$(this)).hide();}else{$(this).show();}}});}};ngutils.visual_link={};ngutils.visual_link=function(node){this.merge=false;this.node=node;this.payload=JSON.parse(node.getAttribute('data-visual-link'));};ngutils.visual_link.prototype={getNode:function(){return this.node;},getPayload:function(){return this.payload;},getType:function(){return this.payload[0];},getId:function(){return this.payload[1];}};ngutils.visual_link.prototype.constructor=ngutils.visual_link;ngutils.visual_links=function(outer_node){if(outer_node instanceof $){outer_node=outer_node.get(0);}
this.outer_node=outer_node;this.pointer=ngutils.visual_links.get_or_generate_collection_pointer(outer_node);this.fired=false;this.setOptions();};ngutils.visual_links.prototype={setOptions:function(options){options=undefined===options?{}:options;let self=this;let getVal=(name,_default)=>{let prev=ngutils.val_from_object(self,name,null);if(null!==prev&&self.merge){_default=prev;}
return ngutils.val_from_object(options,name,_default);};this.container_name=getVal('container_name','ul');this.links=[];this.events={onComplete:getVal('events.onComplete'),onEmptyContainer:getVal('events.onEmptyContainer'),};this.destroy_when_fired=getVal('destroy_when_fired',true);this.missing=null;this.empty_containers=[];this.component_params=getVal('component_params',null);this.suitabilities=getVal('suitabilities',null);this.include_all_suitabilities=getVal('include_all_suitabilities',false);this.send_to_groups=getVal('send_to_groups',true);this.partial_options={checkNode:getVal('partial_options.checkNode',null),checkPartial:getVal('partial_options.checkPartial',null),};this.report_missing=getVal('report_missing',false);this.remove_missing=getVal('remove_misssing',true);this.use_partials=getVal('use_partials',true);this.remove_unpublished=getVal('remove_unpublished',false);this.remove_unsuitable=getVal('remove_unsuitable',false);this.remove_matching_unsuitable=getVal('remove_matching_unsuitable',true);},add:function(node){ngutils.visual_links.generate_id(node);let link=new ngutils.visual_link(node);this.links.push(link);},addNodes:function(outer_node){if(undefined===outer_node||null===outer_node)return;let nodes=ngutils.dom.nodes(outer_node.querySelectorAll('[data-visual-link]'));if(!nodes.length)return;let self=this;if(!outer_node.hasAttribute('data-visual-link-collection')){let pointer=ngutils.visual_links.get_or_generate_collection_pointer(outer_node);self.pointer=pointer;}
for(let n in nodes){self.add(nodes[n]);}
self.sendToGroups(outer_node);},embelish:function(response){let self=this;let report_missing=self.report_missing;if(undefined===response.partials)return;let links=self.links;let missing=[];const removeLink=(link)=>{let node=link.node;let parentNode=node.parentNode;if(self.remove_missing){node.remove();}
if(self.remove_missing&&parentNode.tagName.toLowerCase()===self.container_name&&!parentNode.children.length){if(!self.events.hasOwnProperty('onEmptyContainer')){parentNode.remove();}else{self.empty_containers.push(parentNode);}}
if(report_missing){missing.push(link.getPayload());}
ngutils.event.dispatch('ngutils-dom-change',{info:'visual link removal',node:node,parentNode:parentNode,});};for(let i in links){let link=links[i],node=link.node,nextSibling,partial,partial_group;partial_group=response.partials[link.getType()]?response.partials[link.getType()]:null;if(null===partial_group){removeLink(link);continue;}
if(self.use_partials){partial=partial_group[link.getId()]?partial_group[link.getId()]:null;if(null===partial){removeLink(link);continue;}
let $partial=$(partial);if(null!==self.partial_options.checkNode){if(!self.partial_options.checkNode(node)){removeLink(link);continue;}}
if(null!==self.partial_options.checkPartial){if(!self.partial_options.checkPartial($partial)){removeLink(link);continue;}}
if($partial.hasClass('visual-link-unsuitable')&&self.remove_matching_unsuitable&&null!==self.suitabilities){let suitability=$partial.data('suitability');if(undefined!==suitability&&!self.suitabilities.includes(suitability)){removeLink(link);continue;}}
if(($partial.hasClass('visual-link-unpublished')&&self.remove_unpublished)||($partial.hasClass('visual-link-unsuitable')&&self.remove_unsuitable)){removeLink(link);continue;}
$(node).replaceWith($partial);}}
if(missing.length&&self.report_missing){self.missing=missing;}
self.processEvents(response);},removeUnpublished:function(){if(false===self.remove_unpublished)return;},processEvents:function(response){let self=this;if(null!==self.events.onComplete){self.events.onComplete.call(self,response);}
if(null!==self.events.onEmptyContainer){for(let i in self.empty_containers){self.events.onEmptyContainer(self.empty_containers[i]);}
self.empty_containers=[];}},sendToGroups:function(outer_node){if(!this.send_to_groups)return;let self=this;let containers=ngutils.dom.nodes(outer_node.querySelectorAll('[data-visual-link]'));let container=null;for(let i=0,j,len=containers.length;i<len;++i){let node=containers[i];if(null===container){container=document.createElement(self.container_name);container.setAttribute('class','itemlist alternating');node.parentNode.insertBefore(container,node);}
j=i+1;let has_more=(j<len&&node.nextSibling&&node.nextSibling===containers[j]);container.append(node);if(!has_more){container=null;}}},load:function(){let self=this;let formData=new FormData();let payload=[];for(let i in this.links){payload.push(this.links[i].getPayload());}
if(!payload.length)return;formData.set('payload',JSON.stringify(payload));const d=PHP.get('current_domain');const url=`https://${d}/visual-links-fetch`;let data={'ids':JSON.stringify(payload),'component_params':self.component_params,'include_all_suitabilities':self.include_all_suitabilities?1:0,};if(self.include_all_suitabilities){data.userkey=PHP.get('uek');}
$.postXD(url,data,(response)=>{self.embelish(response);});}};ngutils.visual_links.prototype.constructor=ngutils.visual_links;ngutils.visual_links.generate_id=(function(){let _id=0;return function(element){if(!element.id){element.id="visual_link_temp_id_"+_id++;}
return element.id;};}());ngutils.visual_links.get_collection_pointer=(outer_node)=>{if(outer_node instanceof $){outer_node=outer_node.get(0);}
let pointer=outer_node.dataset.visualLinkCollection;if(undefined===pointer||null===pointer){return null;}
return parseInt(pointer,10);};ngutils.visual_links.get_or_generate_collection_pointer=(outer_node)=>{if(outer_node instanceof $){outer_node=outer_node.get(0);}
let pointer=ngutils.visual_links.get_collection_pointer(outer_node);if(null===pointer){pointer=ngutils.visual_links.generate_collection_pointer(outer_node);}
return pointer;};ngutils.visual_links.generate_collection_pointer=(function(){let _id=0;return function(element){let cur=_id;element.setAttribute('data-visual-link-collection',_id++);return cur;};}());ngutils.visual_links.instance=null;ngutils.visual_links.instances={};ngutils.visual_links.getInstance=(outer_node)=>{if(undefined===outer_node)throw"Must get outer node here!";let pointer=ngutils.visual_links.get_or_generate_collection_pointer(outer_node);if(!ngutils.visual_links.instances.hasOwnProperty(pointer)){ngutils.visual_links.instances[pointer]=new ngutils.visual_links(outer_node);}
return ngutils.visual_links.instances[pointer];};ngutils.visual_links.add_from_dom=(outer_node,options,merge)=>{merge=merge||false;if(null===outer_node)return;let instance=ngutils.visual_links.getInstance(outer_node);instance.merge=merge;if(undefined!==options){if(null===options)options={};instance.setOptions(options);}
instance.addNodes(outer_node);let p=new Promise((resolve,reject)=>{if(instance.fired){return reject(instance);}
instance.load();resolve(instance);}).then((instance)=>{instance.fired=false;});};})(jQuery);}